...
public class AdminPage extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<String> nameCategory = pdao.getAllNameCate();

        Map<String, Integer> mapPhone = new LinkedHashMap<>();

        for (int i = 0; i < nameCategory.size(); i++) {
            int count = pdao.getCountCateByName(nameCategory.get(i));
            mapPhone.put(nameCategory.get(i), count);
        }
        
        
        
        for (Map.Entry<String, Integer> entry : mapPhone.entrySet()) {
            Object key = entry.getKey();
            Object val = entry.getValue();   
            request.setAttribute((String) key, (int)val);
        }
